...
export const metadata: Metadata = {
  ...
  description: "View and download Aarnav Scientific's ISO 9001:2015 and MSME (Udyam) certifications.",
};

export default async function CertificationsPage() {
  const certificates = await prisma.certificate.findMany({ orderBy: { sortOrder: "asc" } }).catch(() => []);
        ...
        <div className="grid gap-8 sm:grid-cols-2">
          {certificates.map((c) => (
            <div key={c.id} className="card-surface overflow-hidden p-0">
              <div className="relative aspect-[3/4] bg-navy-50">
