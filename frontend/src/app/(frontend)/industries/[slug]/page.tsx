...
async function getIndustry(slug: string) {
  try {
    const fromDb = await prisma.industry.findUnique({ where: { slug } });
...
export default async function IndustryDetailPage({ params }: { params: { slug: string } }) {
  ...
  let products = relevantCategorySlugs
    ? await prisma.product
        .findMany({
          where: { isActive: true, category: { slug: { in: relevantCategorySlugs } } },
          take: 4,
  ...
  if (products.length === 0) {
    products = await prisma.product
      .findMany({ where: { isActive: true }, take: 4, orderBy: { isFeatured: "desc" } })
      .catch(() => []);
  }
