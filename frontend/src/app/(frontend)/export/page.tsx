...
const getCountryNames = cache(async () => {
  const dbCountries = await prisma.country
    .findMany({ where: { isServed: true }, orderBy: { sortOrder: "asc" } })
    .catch(() => []);
  return dbCountries.length > 0 ? dbCountries.map((c) => c.name) : FALLBACK_COUNTRIES;
});
