...
import type { ProductCardData, ProductListResponse } from "@/common/api-types";
...
async function fetchFeatured(): Promise<ProductCardData[]> {
  const data = await backendFetch<ProductListResponse>("/api/products?featured=true&limit=8");
  return data.items || [];
}
