...
export async function CertificatesSection() {
  ...
  let catalogueItems: CatalogueItemData[] = [];
  try {
    [certificates, catalogueItems] = await Promise.all([getCerts(), getCatalogueItems()]);
  } catch {
    certificates = [];
    catalogueItems = [];
  }

  if (certificates.length === 0 && catalogueItems.length === 0) return null;
    ...
    <section className="section-y bg-navy-50/50">
      <div className="container-px mx-auto max-w-7xl">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <Link href="/downloads" className="flex items-center gap-2 text-sm font-semibold text-teal-700 hover:underline">
...
async function getCatalogueItems(): Promise<CatalogueItemData[]> {
