...
export default async function CertificationsPage() {
  const [certificates, settings]: [CertificateData[], Record<string, string>] = await Promise.all([
    backendFetch<{ certificates: CertificateData[] }>("/api/certificates")
      .then((data) => data.certificates || [])
      .catch((): CertificateData[] => []),
    backendFetch<SettingsResponse>("/api/settings/public?key=product_catalogue_url")
      .then((data) => data.settings || {})
      .catch((): Record<string, string> => ({})),
  ]);
  const catalogueUrl = settings.product_catalogue_url || DEFAULT_CATALOGUE_URL;
