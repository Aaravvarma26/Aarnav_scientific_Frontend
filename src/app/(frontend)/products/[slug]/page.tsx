...
import type { ProductCardData, ProductDetailResponse } from "@/common/api-types";
import {
  FlaskConical,
  Package,
  FileText,
...
async function getProductData(slug: string) {
  return backendFetch<ProductDetailResponse>(`/api/products/${encodeURIComponent(slug)}`);
}

export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const product = await getProductData(params.slug).then((data) => data.product).catch(() => null);
...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  const data = await getProductData(params.slug).catch(() => null);
  const product = data?.product;
  const related: ProductCardData[] = data?.related || [];
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_1.2fr]">
          ...
          <div>
            ...
            {product.packSizes.length > 0 && (
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.packSizes.map((p: ProductDetailResponse["product"]["packSizes"][number]) => (
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              {product.downloads.map((d: ProductDetailResponse["product"]["downloads"][number]) => (
                <a
                  key={d.id}
                  href={d.fileUrl}
                  target="_blank"
            ...
            {product.specifications.length > 0 && (
              <div className="mt-6 border-t border-navy-100 pt-6">
                ...
                <table className="mt-3 w-full text-sm">
                  <tbody>
                    {product.specifications.map((s: ProductDetailResponse["product"]["specifications"][number]) => (
